...
export default function ReportsPage() {
  ...
  const filteredPOs = state.productionOrders.filter((po) => !plantFilter || po.plant === plantFilter);
  ...
  const handleExportCSV = () => {
    ...
    if (activeTab === 'production') {
      csv = 'PO,Sản phẩm,Trạng thái,Kế hoạch (KG),WIP (KG),FG (KG),Phế (KG),Nhà máy\n';
      filteredPOs.forEach((po) => {
        csv += `${po.id},${po.productName},${po.status},${po.plannedQty},${po.wipQty || 0},${po.fgQty || 0},${po.scrapQty || 0},${po.plant}\n`;
      });
    } else if (activeTab === 'inventory') {
      csv = 'Loại,Mã,Sản phẩm,Số lượng,Đơn vị,Trạng thái,Nhà máy\n';
      filteredHUs.forEach((hu) => {
        csv += `HU,${hu.id},${hu.product},${hu.qty},${hu.unit},${hu.status},${hu.plant}\n`;
      });
    }
    ...
    a.href = url;
    a.download = `antesco-report-${activeTab}-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
  ...
  return (
    <div className="min-h-screen bg-ant-bg flex flex-col">
      <header className="sticky top-0 z-40 bg-ant-card/95 backdrop-blur-xl border-b border-gray-100 px-4 h-14 flex items-center gap-3 shrink-0">
        ...
        <button onClick={handleExportCSV} className="h-9 px-3 rounded-xl bg-ant-sx text-white text-xs font-bold flex items-center gap-1.5 active:scale-95 transition-all">
                ...
                {filteredPOs.map((po) => (
                  <Link key={po.id} to={`/production/detail/${po.id}`}
                    ...
                    <div className="flex items-center gap-3 shrink-0">
                      <span className="text-xs text-ant-text-secondary">{po.plannedQty.toLocaleString()} KG</span>
                ...
                <div className="space-y-1.5">
                  {state.offlineQueue.map((q) => (
                    <div key={q.queueId} className="flex items-center justify-between p-3 rounded-xl bg-ant-bg">
                      ...
                      <span className={`text-xs font-bold px-2 py-1 rounded-full ${q.status === 'Pending' ? 'bg-ant-warning/10 text-ant-warning' : q.status === 'Synced' ? 'bg-ant-sx/10 text-ant-sx' : q.status === 'Failed' ? 'bg-ant-error/10 text-ant-error' : 'bg-ant-offline/10 text-ant-offline'}`}>
                        {q.status}
                      </span>
                    </div>
                  ))}
                ...
                <div className="space-y-1.5">
                  {state.errorQueue.slice(0, 5).map((e) => (
                    <div key={e.id} className="flex items-center justify-between p-3 rounded-xl bg-ant-bg">
                      ...
                      <span className={`text-xs font-bold px-2 py-1 rounded-full shrink-0 ml-2 ${e.status === 'Pending' ? 'bg-ant-warning/10 text-ant-warning' : e.status === 'Need Review' ? 'bg-ant-error/10 text-ant-error' : 'bg-ant-sx/10 text-ant-sx'}`}>
                        {e.status}
                      </span>
                    </div>
                  ))}
